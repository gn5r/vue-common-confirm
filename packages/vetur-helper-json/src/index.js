...
const VConfirm = require('@gn5r/vue-common-confirm')
...
const attributes = Object.keys(components).reduce((attrs, k) => {
  const tmp = components[k].props.reduce((a, prop) => {
    ...
    if (!type) type = ''
    else if (Array.isArray(type)) { type = type.map((t) => t.toLowerCase()).join('|') } else type = type.toLowerCase()
...
console.debug('tags.jsonとattributes.jsonを作成しました')
